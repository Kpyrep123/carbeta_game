...
function WaveOfTerrorVision( keys )
	...
	local wave_width = ability:GetLevelSpecialValueFor("wave_width", (ability:GetLevel() - 1))
		local bonus_range = caster:GetCastRangeIncrease()

	local wave_range = ability:GetLevelSpecialValueFor("wave_range", (ability:GetLevel() - 1)) + caster:GetTalentValue("special_bonus_titan_wave_range")
	...
	local projectileTable =
	{
		...
		fDistance = wave_range + bonus_range,
...
function buff( keys )
	local caster = keys.caster
	local ability = keys.ability
	local Duration = ability:GetSpecialValueFor("tooltip_duration")
	local modifierName = "modifier_damage_bonus"
