...
function FierySoul( keys )
	...
	local maxStack = ability:GetLevelSpecialValueFor("fiery_soul_max_stacks", (ability:GetLevel() - 1)) + caster:GetTalentValue("special_bonus_unique_flaw_squal")
	local modifierCount = caster:GetModifierCount()
	local currentStack = 0
	local modifierBuffName = "modifier_lsa_buff_datadriven"
	local modifierStackName = "modifier_lsa_buff_stack_datadriven"
	local modifierName
