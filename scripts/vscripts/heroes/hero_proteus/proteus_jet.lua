...
function NovaMotion(keys)
	...
	if ability.traveled_distance < ability.distance then
		caster:SetAbsOrigin(caster:GetAbsOrigin() + ability.direction * ability.speed)
		ability.traveled_distance = ability.traveled_distance + ability.speed
		particle_jet = ParticleManager:CreateParticle("particles/units/heroes/hero_mars/mars_arena_of_blood_impact_warp.vpcf", PATTACH_ABSORIGIN, caster)
	else
		...
		-- Remove the motion controller once the distance has been traveled
		caster:InterruptMotionControllers(true)
		caster:RemoveModifierByName("modifier_nova_dash")
	end
end
...
function invis( keys )
	...
	local duration = 2
	if caster:HasShard() then 
	ability:ApplyDataDrivenModifier(caster, caster, "modifier_nova_cloack", { Duration = duration})
end
end
