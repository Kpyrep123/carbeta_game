...
function destroy (keys) -- KV OnDestroy
	...
	local caster = keys.caster
	local ability = keys.ability
	local dmg = ability:GetLevelSpecialValueFor("damage",ability:GetLevel()-1)  + caster:GetTalentValue("spec_bonus_uther_bolt_dmg")
	local explosionRadius = ability:GetSpecialValueFor("explosion_radius") + caster:GetTalentValue("spec_bonus_uther_bolt_rad")
