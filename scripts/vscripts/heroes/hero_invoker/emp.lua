...
function invoker_emp_datadriven_on_spell_start(keys)
	...
	if wex_ability ~= nil then		
		...
		local emp_dummy_unit = CreateUnitByName("npc_dummy_unit_inv", target_point, false, nil, nil, keys.caster:GetTeam())
		...
		Timers:CreateTimer({
			...
			callback = function()
				...
				local emp_explosion_effect = ParticleManager:CreateParticle("particles/units/heroes/hero_leshrac/leshrac_split_earth.vpcf",  PATTACH_ABSORIGIN, emp_dummy_unit)
				...
				for i, individual_unit in ipairs(nearby_enemy_units) do
					...
					individual_unit:ApplyDataDrivenModifier(caste, handle target, string modifier_name, handle modifierArgs)
