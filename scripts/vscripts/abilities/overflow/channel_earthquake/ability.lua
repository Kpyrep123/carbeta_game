...
LinkLuaModifier( "generic_lua_stun", "abilities/overflow/generic_stun.lua", LUA_MODIFIER_MOTION_NONE )
 ...
 function channel_earthquake:Explosion(vPos)
	 ...
	 local particleName = "particles/econ/items/luna/luna_lucent_ti5_gold/luna_eclipse_impact_notarget_moonfall_gold.vpcf"
	...
	EmitSoundOnLocationWithCaster(self.point, "Ability.LagunaBladeImpact", hCaster )
		local damage = {
		...
		if #enemies > 0 then
			for _,enemy in pairs(enemies) do
				if enemy ~= nil and ( not enemy:IsMagicImmune() ) and ( not enemy:IsInvulnerable() ) then
					...
					enemy:AddNewModifier( self:GetCaster(), self, "generic_lua_stun", { duration = stun_dur , stacking = 0 } )
					damage.victim = enemy
					ApplyDamage( damage )
				end
			end
