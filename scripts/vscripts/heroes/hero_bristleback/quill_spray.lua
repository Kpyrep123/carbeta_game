...
function stack_created(params)
	...
	local base_damage = ability:GetLevelSpecialValueFor("base_damage", ability:GetLevel() - 1) + caster:GetTalentValue("proteus_wave_base_damage")
	local max_damage = ability:GetLevelSpecialValueFor("max_damage", ability:GetLevel() - 1) + caster:GetTalentValue("proteus_wave_max")
	...
	local final_damage = base_damage + (params.stack_damage * modifier_ui_handle:GetStackCount())

	-- If the total damage exceeds the defined maximum damage, clamp it.
	if final_damage > max_damage then
	...
	modifier_ui_handle:SetDuration(params.stack_duration*(1 - params.target:GetStatusResistance()), true)
end
...
function determine_debuff(params)
	if params.target:IsHero() then
		params.ability:ApplyDataDrivenModifier(params.caster, params.target, "modifier_slow", {duration = params.stack_duration})
...
function shard_update( keys )
	local caster = keys.caster
	local ability = caster:FindAbilityByName("wave_dance") 
	if caster:HasModifier("modifier_item_aghanims_shard") then 
		ability:ApplyDataDrivenModifier(caster, caster, "modifier_razor_wave", {})
	end
end