...
function angel_arena_archmage_anomaly:OnSpellStart()
	...
	self.duration = self:GetSpecialValueFor("duration") + self:GetCaster():GetTalentValue("anomaly_dur")
...
function angel_arena_archmage_anomaly:GetAOERadius()
	return self:GetSpecialValueFor( "radius" ) + self:GetCaster():GetTalentValue("anomaly_radius")
end
--------------------------------------------------------------------------------------------------------
...
function modifier_angel_arena_archmage_anomaly_thinker:OnCreated( kv )
	self.aura_radius = self:GetAbility():GetSpecialValueFor( "radius" ) + self:GetCaster():GetTalentValue("special_bonus_unquie_lanaya_sparks_radius")
	if IsServer() then
		...
		self.FXIndex = ParticleManager:CreateParticle( "particles/units/heroes/hero_faceless_void/faceless_void_chronosphere_glow_2.vpcf", PATTACH_ABSORIGIN, self:GetParent() )
	...
	self.AlreadyHit = {}
	self.blackList = {["shadow_shaman_shackles"] = true, ["enigma_demonic_conversion"] = true, ["witch_doctor_paralyzing_cask"] = true, ["wisp_tether_datadriven"] = true, ["sandra_sacrifice"] = true, }
