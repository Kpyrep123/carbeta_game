...
function Impact(keys)
	...
	local target = keys.target
	local ability = keys.ability
	local radius = ability:GetLevelSpecialValueFor("radius", (ability:GetLevel() -1)) + caster:GetTalentValue("telekinesis_radius")
	local stun_duration = ability:GetLevelSpecialValueFor("stun_duration", (ability:GetLevel() -1)) + caster:GetTalentValue("telekinesis_stun_dur")
