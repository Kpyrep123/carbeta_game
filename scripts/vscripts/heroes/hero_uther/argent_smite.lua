function HealingAttack (keys) 
	...
	local Heal_Factor = ability:GetSpecialValueFor("Heal_Factor") + caster:GetTalentValue("spec_bonus_uther_heal")
	local Tower_Heal_Factor = ability:GetSpecialValueFor("Tower_Heal_Factor") + caster:GetTalentValue("spec_bonus_uther_heal")
	if caster == attacker then -- only the modifier owner should heal, else enemies would heal too
		if target:IsBuilding() then
			local healAmount = target:GetHealth() * Tower_Heal_Factor / 100
			target:Heal(healAmount,caster)
		else
			local healAmount = target:GetHealth() * Heal_Factor / 100
