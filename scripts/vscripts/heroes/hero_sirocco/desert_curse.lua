function ScepterCheck( keys )
  ...
  local duration = ability:GetSpecialValueFor("duration") + caster:GetTalentValue("special_bonus_unique_sirocco_ult_dur")
  local scepter_duration = ability:GetSpecialValueFor("scepter_duration") + caster:GetTalentValue("special_bonus_unique_sirocco_ult_dur")
    if target:TriggerSpellAbsorb(ability) then
      RemoveLinkens(target)
      return
    end
  if caster:HasScepter() then
    ability:ApplyDataDrivenModifier(caster, target, plague, {Duration = scepter_duration* (1- target:GetStatusResistance())})
  else
    ability:ApplyDataDrivenModifier(caster, target, plague, {Duration = duration* (1- target:GetStatusResistance())})
  end
end
...
function damage_cursed( keys )
  ...
  local radius = ability:GetSpecialValueFor("blast_radius") + caster:GetTalentValue("special_bonus_unique_sirocco_ult_rad")
...
function stacks_attack( event )
  ...
  local manaBurn = ability:GetLevelSpecialValueFor("mana_damage_lost", (ability:GetLevel() - 1)) + caster:GetTalentValue("special_bonus_unique_sirocco_ult_mana_loss")
