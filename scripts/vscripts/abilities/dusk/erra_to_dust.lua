...
if IsServer() then
	...
	function erra_to_dust:OnSpellStart()
		...
		EmitSoundOn("Hero_Visage.SoulAssumption.Target", caster)
		local damage = (self:GetSpecialValueFor("hp_to_bonus_damage") + caster:GetTalentValue("special_bonus_unquie_to_dust_dmg"))/100

		local creep_damage = (self:GetSpecialValueFor("creep_hp_to_bonus_damage") + caster:GetTalentValue("special_bonus_unquie_to_dust_dmg"))/100

		local radius = self:GetSpecialValueFor("radius") + caster:GetTalentValue("special_bonus_unquie_to_dust_rad")

		local duration = self:GetSpecialValueFor("duration") + caster:GetTalentValue("special_bonus_unquie_to_dust_dur")
		...
		for k,v in pairs(enemy_found) do
			...
			ParticleManager:CreateParticle("particles/spell_lab/void_step_end.vpcf", PATTACH_ABSORIGIN_FOLLOW, v) --[[Returns:int
