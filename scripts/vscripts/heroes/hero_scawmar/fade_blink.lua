...
function Hideability( keys )
	local caster = keys.caster
	local ability = keys.ability
	local ab = caster:FindAbilityByName("hylonome_eldritch_pull")  
...
function HideabilityHild( keys )
	local caster = keys.caster
	local ability = keys.ability
	local ab = caster:FindAbilityByName("scawmar_spirits_away")  
	if not caster:HasModifier("modifier_item_aghanims_shard") then
		ab:SetHidden(true)
	else
		ab:SetHidden(false)
...
function HideabilityStrider( keys )
	local caster = keys.caster
	local ability = keys.ability
	local ab = caster:FindAbilityByName("void_spirit_astral_step_lua")  
	if caster:HasScepter() then
		ab:SetHidden(false)
	else
		ab:SetHidden(true)
	end
	local subab = caster:FindAbilityByName("void_spirit_astral_step_lua")
	subab:SetLevel(1)
end