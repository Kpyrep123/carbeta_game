...
function COverthrowGameMode:OnGameRulesStateChange()
	...
	if nNewState == DOTA_GAMERULES_STATE_PRE_GAME then
		...
		elseif numberOfPlayers > 4 and numberOfPlayers <= 7 then
			...
			nCOUNTDOWNTIMER = 3600
		elseif numberOfPlayers > 9 and numberOfPlayers < 11 then
			--self.TEAM_KILLS_TO_WIN = 20
			nCOUNTDOWNTIMER = 3600
		else
...
--------------------------------------------------------------------------- 
function COverthrowGameMode:OnEntityKilled( event , unit )
	...
	if killedUnit:IsRealHero() then
		...
		if hero:IsRealHero() and heroTeam ~= killedTeam then
			...
			if killedUnit:GetTeam() == self.leadingTeam and self.isGameTied == false then
				...
				PlayerResource:ModifyGold( memberID, 150, true, 0 )
				hero:AddExperience( 0, 0, false, false )
		...
		if killedUnit:HasItemInInventory("item_aegis") then 
								COverthrowGameMode:SetRespawnTime( killedTeam, killedUnit, 3 )
							end
	end
end
...
function COverthrowGameMode:SetRespawnTime( killedTeam, killedUnit, extraTime )
	...
	if killedTeam == self.leadingTeam and self.isGameTied == false then
		killedUnit:SetTimeUntilRespawn( (20 + extraTime) + (killedUnit:GetLevel()*1.3) )
	else
		killedUnit:SetTimeUntilRespawn( (10 + extraTime) + (killedUnit:GetLevel()*1.3) )
	end
end
