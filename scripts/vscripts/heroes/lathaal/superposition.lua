LinkLuaModifier("modifier_superposition","scripts/vscripts/heroes/lathaal/superposition.lua",LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_superposition_tracker","scripts/vscripts/heroes/lathaal/superposition.lua",LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_superposition_illusion","scripts/vscripts/heroes/lathaal/modifiers/modifier_superposition_illusion.lua",LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_superposition_trail","scripts/vscripts/heroes/lathaal/superposition.lua",LUA_MODIFIER_MOTION_NONE)
...
function lathaal_superposition:OnSpellStart()
	if IsServer() then
		...
		local particle = ParticleManager:CreateParticleForTeam("particles/units/heroes/hero_lathaal/super_position_base.vpcf", PATTACH_POINT_FOLLOW, caster, caster:GetTeamNumber())
		trail:AddParticle(particle, false, false, 1, true, false)

		if point then 
			local distance = (point - origin):Length2D()
			distance = math.min(distance, self:GetCastRange(point, caster) + caster:GetCastRangeIncrease())
...
function lathaal_superposition:ReleaseSuperpositionIllusion(keys)
	...
	illusion:AddNewModifier(caster,self,"modifier_illusion", {
		...
		incoming_damage = self:GetSpecialValueFor("incoming_damage") + caster:GetTalentValue("special_bonus_unique_lathaal_4")
	})
...
function modifier_superposition:OnDestroy()
	...
	local maxCharges = self:GetAbility():GetSpecialValueFor("charges") + self:GetCaster():GetTalentValue("special_bonus_unique_lathaal_5")
...
function modifier_superposition:OnAbilityExecuted(event)
	...
	if event.unit == self:GetCaster() and self:GetCaster():IsRealHero() and not self:GetCaster():HasModifier("modifier_superposition_illusion") then
		...
		if ability:GetAbilityName() == "item_refresher" or ability:GetAbilityName() == "item_refresher_shard" then
			local charges = self:GetAbility():GetSpecialValueFor("charges") + self:GetCaster():GetTalentValue("special_bonus_unique_lathaal_5")
