LinkLuaModifier("modifier_divine_intervention", "scripts/vscripts/heroes/lathaal/divine_intervention.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_divine_intervention_armor", "scripts/vscripts/heroes/lathaal/divine_intervention.lua", LUA_MODIFIER_MOTION_NONE)
...
function lathaal_divine_intervention:OnSpellStart()
	...
	local modifier = target:AddNewModifier(caster, self, "modifier_divine_intervention", {duration = self:GetSpecialValueFor("channel_time"), break_distance = self:GetSpecialValueFor("break_distance") + caster:GetCastRangeIncrease()})

	local ministun = target:AddNewModifier(caster, self, "modifier_stunned", {duration = 0.25})

	local particle = ParticleManager:CreateParticle("particles/units/heroes/hero_lathaal/divine_intervention.vpcf", PATTACH_POINT_FOLLOW, caster)
