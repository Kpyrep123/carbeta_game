...
function spell_lab_souls_base_modifier:OnCreated(kv)
	...
	if IsServer() then
		...
		if (self.bDrop) then
			...
			self.nFXIndex = ParticleManager:CreateParticle( "particles/econ/items/bloodseeker/bloodseeker_eztzhok_weapon/bloodseeker_bloodrage_eztzhok_ovr.vpcf", PATTACH_ABSORIGIN, self:GetParent() )
			local Colour = self:GetColour()
