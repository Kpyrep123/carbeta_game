...
hoodwink_sharpshooter_custom.creep_init = 0.1
hoodwink_sharpshooter_custom.creep_inc = 1.0
...
hoodwink_sharpshooter_custom.hit_init = 0
hoodwink_sharpshooter_custom.hit_inc = 10

hoodwink_sharpshooter_custom.hit_cd = 0.7
...
function hoodwink_sharpshooter_custom:OnProjectileHit_ExtraData( target, location, ExtraData )
	...
	if not self:GetCaster():HasTalent("hoodwink_sharp_2") then 
		if not sound or sound:IsNull() then return end
	...
	if not target then 
		if reduce_cd == true and self:GetCaster():HasModifier("modifier_hoodwink_sharp_6") then 
	...
	local k = 1
	local creep = true
	if self:GetCaster():HasTalent("hoodwink_sharp_2") then 
		creep = false
		if not target:IsHero() then 
			k = self.creep_init + self.creep_inc
		end
	end
...
function modifier_hoodwink_sharpshooter_custom:GetEffectName()
if self:GetCaster():HasScepter() then
...
function modifier_hoodwink_sharpshooter_custom:OnCreated( kv )
	...
	self.damage = self:GetAbility():GetSpecialValueFor( "max_damage" ) + self.caster:GetTalentValue("special_bonus_unquie_sharo_max")

	if self:GetCaster():HasModifier("modifier_hoodwink_sharpshooter_custom_hits") and self:GetCaster():HasModifier("special_bonus_Sharp_1") then 
	...
	self.info = {
	    ...
	    EffectName = "particles/units/heroes/hero_windrunner/windrunner_spell_powershot_rainmaker.vpcf",
	...
	local effect_cast = ParticleManager:CreateParticle( "particles/econ/items/windrunner/windranger_arcana/windranger_arcana_frontpage_ambient.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent )
...
if not IsServer() then return end
	...
	self.info.ExtraData = { damage = self.damage * pct, pct = pct, duration = self.duration * pct, x = direction.x, y = direction.y, sound = sound:entindex(), }
...
function modifier_hoodwink_sharpshooter_custom:OnIntervalThink()
	...
	if self:GetCaster():HasScepter() and self:GetAbility():GetAutoCastState() == false then 
