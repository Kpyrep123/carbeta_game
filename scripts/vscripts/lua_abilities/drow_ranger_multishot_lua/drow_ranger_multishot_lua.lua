...
function drow_ranger_multishot_lua:GetCooldown( level )
	local upgrade_cooldown = 0 
	local talent = self:GetCaster():FindAbilityByName("special_bonus_unquie_mult_cd")
...
function drow_ranger_multishot_lua:OnProjectileHit_ExtraData( target, location, data )
	...
	-- get value
	local damage = self:GetSpecialValueFor( "arrow_damage_pct" ) + self:GetCaster():GetTalentValue("special_bonus_unquie_multishot_damage")
