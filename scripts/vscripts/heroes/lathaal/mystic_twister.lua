LinkLuaModifier("modifier_mystic_twister_thinker", "scripts/vscripts/heroes/lathaal/mystic_twister.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_mystic_twister_thinker:OnCreated( keys )
	...
	self.damage = (ability:GetAbilityDamage() + self.caster:GetTalentValue("special_bonus_unique_lathaal_2")) * interval
	self.damageType = ability:GetAbilityDamageType()
	self.radius = ability:GetSpecialValueFor("radius")

	local particle = ParticleManager:CreateParticle("particles/units/heroes/hero_lathaal/mystic_twister.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function modifier_mystic_twister_thinker:OnIntervalThink()
	...
	for k, unit in pairs(units) do
		...
		local hitParticle = ParticleManager:CreateParticle("particles/units/heroes/hero_lathaal/mystic_twister_damage.vpcf", PATTACH_ABSORIGIN, unit)
