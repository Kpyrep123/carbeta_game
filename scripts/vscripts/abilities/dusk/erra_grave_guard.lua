...
function modifier_grave_guard_recovery:GetModifierConstantHealthRegen()
	return self:GetAbility():GetSpecialValueFor("hp_recovery") * self:GetCaster():GetMaxHealth() / 100
end

function modifier_grave_guard_recovery:GetModifierConstantManaRegen()
	return self:GetAbility():GetSpecialValueFor("mp_recovery") * self:GetCaster():GetMaxMana() / 100
end