...
function ability_tidehunter_anchor_smash:OnSpellStart()
    ...
    local radius = self:GetSpecialValueFor("radius") + self:GetCaster():GetTalentValue("spec_jugg_smash_radius")
    ...
    local fx = ParticleManager:CreateParticle("particles/econ/items/axe/axe_weapon_bloodchaser/axe_attack_blur_counterhelix_bloodchaser.vpcf", PATTACH_ABSORIGIN, caster)
...
function modifier_ability_tidehunter_anchor_smash_reduction:OnCreated()
    self.damage_reduction = self:GetAbility():GetSpecialValueFor("damage_reduction") + self:GetCaster():GetTalentValue("spec_jugg_smash_slow")
...
function modifier_ability_tidehunter_anchor_smash_reduction:GetModifierMoveSpeedBonus_Percentage() return self.damage_reduction end