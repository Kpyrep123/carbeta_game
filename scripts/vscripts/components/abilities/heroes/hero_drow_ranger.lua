...
----------------------------
--		FROST ARROWS      --
---------------------------- modifier_imba_drow_ranger_marksmanship_723_shard modifier_imba_drow_ranger_marksmanship_723_shard_range
...
LinkLuaModifier("modifier_imba_drow_ranger_marksmanship_723_shard", "components/abilities/heroes/hero_drow_ranger", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_drow_ranger_marksmanship_723_shard_aura", "components/abilities/heroes/hero_drow_ranger", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_drow_ranger_marksmanship_723_shard_range", "components/abilities/heroes/hero_drow_ranger", LUA_MODIFIER_MOTION_NONE)
...
function modifier_imba_frost_arrows_thinker:OnAttackLanded(keys)
	if IsServer() then
		...
		if self.caster == attacker then
			...
			local instakill_chance = 0 + self.caster:GetTalentValue("special_bonus_imba_drow_ranger_2")
...
function imba_drow_ranger_gust:GetCastRange(location, target)
	return self:GetSpecialValueFor("wave_distance") + (self:GetCaster():GetTalentValue("special_bonus_imba_drow_ranger_9"))
end
...
function imba_drow_ranger_gust:OnSpellStart()
	...
	local wave_distance = ability:GetSpecialValueFor("wave_distance") + caster:GetTalentValue("special_bonus_imba_drow_ranger_9")
...
function imba_drow_ranger_gust:OnProjectileHit(target, location)
	if IsServer() then
		...
		local max_distance = ability:GetSpecialValueFor("max_distance") + caster:GetTalentValue("special_bonus_imba_drow_ranger_9")
...
function imba_drow_ranger_multishot:OnProjectileHit_ExtraData(target, location, ExtraData)
	...
	if target and not self.targets_hit[ExtraData.volley_index][target:entindex()] then
		...
		if self:GetCaster():HasShard() and target:IsRealHero() then 
			local bkb = self:GetCaster():AddNewModifier(self:GetCaster(), self, "imba_drow_ranger_multishot_shard", {duration = 1})
			bkb:SetDuration(bkb:GetRemainingTime() + 0.25, true)
		end
...
LinkLuaModifier("imba_drow_ranger_multishot_shard", "components/abilities/heroes/hero_drow_ranger", LUA_MODIFIER_MOTION_NONE)

imba_drow_ranger_multishot_shard = class({})
function imba_drow_ranger_multishot_shard:IsHidden() return false end
function imba_drow_ranger_multishot_shard:IsPurgable() return false end


function imba_drow_ranger_multishot_shard:OnCreated()
	local prt = ParticleManager:CreateParticle("particles/drow_ranger_bkb_talent_mid.vpcf", PATTACH_CUSTOMORIGIN, self:GetParent())
	ParticleManager:SetParticleControlEnt(prt, 2, self:GetCaster(), PATTACH_POINT_FOLLOW, "attach_hitloc", self:GetCaster():GetAbsOrigin(), true)
	self:AddParticle(prt, false, false, -1, true, false)
end

function imba_drow_ranger_multishot_shard:OnRefresh()

end

function imba_drow_ranger_multishot_shard:CheckState()
...
function modifier_imba_drow_ranger_multishot:OnCreated()
	...
	self.arrow_damage_pct		= self:GetAbility():GetSpecialValueFor("arrow_damage_pct") + self:GetCaster():GetTalentValue("special_bonus_unique_drow_ranger_1")
...
function imba_drow_ranger_marksmanship_723:GetBehavior()
	if self:GetCaster():HasTalent("special_bonus_imba_drow_ranger_5") then
...
function imba_drow_ranger_marksmanship_723:GetCastRange(  )
	if self:GetCaster():HasTalent("special_bonus_imba_drow_ranger_5") then
		return 900
	else
		 return 400
...
function imba_drow_ranger_marksmanship_723:GetCastPoint(  )
	return 0.3
end

function imba_drow_ranger_marksmanship_723:GetCooldown(  )
	if self:GetCaster():HasTalent("special_bonus_imba_drow_ranger_5") then
		return 60
	else
		 return 0
...
function imba_drow_ranger_marksmanship_723:GetCastAnimation(  )
	return ACT_DOTA_CAST_ABILITY_2
end

function imba_drow_ranger_marksmanship_723:GetAOERadius(  )
	return 450
end 

function imba_drow_ranger_marksmanship_723:OnSpellStart(  )
	self:GetCaster():EmitSound("Hero_Ancient_Apparition.IceVortexCast")
	local freezing_field = CreateModifierThinker(self:GetCaster(), self, "modifier_imba_drow_ranger_marksmanship_723_shard", {duration = 5}, self:GetCursorPosition(), self:GetCaster():GetTeamNumber(), false)

end

modifier_imba_drow_ranger_marksmanship_723_shard = class({})
function modifier_imba_drow_ranger_marksmanship_723_shard:IsHidden() return false end
function modifier_imba_drow_ranger_marksmanship_723_shard:GetAuraRadius()
	return 600 --global
end

function modifier_imba_drow_ranger_marksmanship_723_shard:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_NONE
end

function modifier_imba_drow_ranger_marksmanship_723_shard:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_ENEMY
end

function modifier_imba_drow_ranger_marksmanship_723_shard:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end

function modifier_imba_drow_ranger_marksmanship_723_shard:GetModifierAura()
	return "modifier_imba_drow_ranger_marksmanship_723_shard_aura"
end


function modifier_imba_drow_ranger_marksmanship_723_shard:IsAura()
	return true
end
function modifier_imba_drow_ranger_marksmanship_723_shard:IsPurgable() return false end
function modifier_imba_drow_ranger_marksmanship_723_shard:GetTexture() return end
function modifier_imba_drow_ranger_marksmanship_723_shard:GetEffectName() return end

function modifier_imba_drow_ranger_marksmanship_723_shard:OnCreated()
	...
	ParticleManager:SetParticleControl(vortex_particle, 5, Vector(600, 0, 0))
	self:AddParticle(vortex_particle, false, false, -1, false, false)
	
	AddFOWViewer(self:GetCaster():GetTeamNumber(), self:GetParent():GetAbsOrigin(), 600, 5, false)
end

function modifier_imba_drow_ranger_marksmanship_723_shard:OnRefresh()

end


modifier_imba_drow_ranger_marksmanship_723_shard_aura = class({})
function modifier_imba_drow_ranger_marksmanship_723_shard_aura:IsHidden() return true end
function modifier_imba_drow_ranger_marksmanship_723_shard_aura:IsPurgable() return false end
function modifier_imba_drow_ranger_marksmanship_723_shard_aura:GetStatusEffectName() return "particles/status_fx/status_effect_wyvern_arctic_burn.vpcf" end

function modifier_imba_drow_ranger_marksmanship_723_shard_aura:OnCreated()

end

function modifier_imba_drow_ranger_marksmanship_723_shard_aura:OnRefresh()

end

function modifier_imba_drow_ranger_marksmanship_723_shard_aura:DeclareFunctions(  )
	return {MODIFIER_EVENT_ON_ORDER}
end

function modifier_imba_drow_ranger_marksmanship_723_shard_aura:OnOrder( params )
	if not IsServer() then return end
	local unit = params.unit
	local target = params.target
	if unit == self:GetCaster() and target == self:GetParent() then 
		self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_imba_drow_ranger_marksmanship_723_shard_range", {})
	end
end

modifier_imba_drow_ranger_marksmanship_723_shard_range = class({})
function modifier_imba_drow_ranger_marksmanship_723_shard_range:IsHidden() return true end
function modifier_imba_drow_ranger_marksmanship_723_shard_range:IsPurgable() return false end
function modifier_imba_drow_ranger_marksmanship_723_shard_range:GetTexture() return end
function modifier_imba_drow_ranger_marksmanship_723_shard_range:GetEffectName() return end

function modifier_imba_drow_ranger_marksmanship_723_shard_range:OnCreated()

end

function modifier_imba_drow_ranger_marksmanship_723_shard_range:OnRefresh()

end

function modifier_imba_drow_ranger_marksmanship_723_shard_range:DeclareFunctions()
	return {MODIFIER_PROPERTY_ATTACK_RANGE_BONUS,
			MODIFIER_EVENT_ON_ORDER,
			MODIFIER_EVENT_ON_ATTACK_START}
end

function modifier_imba_drow_ranger_marksmanship_723_shard_range:GetModifierAttackRangeBonus(  )
	return 9999999
end

function modifier_imba_drow_ranger_marksmanship_723_shard_range:OnOrder( params )
	if IsServer() then
		-- filter
		local unit = params.unit
		local target = params.target
		...
		if unit==self:GetParent() then
			if target~=nil then
				if target:HasModifier("modifier_imba_drow_ranger_marksmanship_723_shard_aura") then
					pass = false
				end
			end
		end
...
function modifier_imba_drow_ranger_marksmanship_723_shard_range:OnAttackStart( params )
	if IsServer() then
		-- filter
		...
		if attacker==self:GetParent() then
			if not target:HasModifier("modifier_imba_drow_ranger_marksmanship_723_shard_aura") then
				pass = true
			end
		end
...
function modifier_imba_drow_ranger_marksmanship_723:OnAttackLanded(keys)
	if keys.attacker == self:GetParent() then
		...
		if self:GetCaster() == attacker then
			...
			-- #2 Talent: Chance to kill creeps instantly
			local instakill_chance = self:GetCaster():GetTalentValue("special_bonus_imba_drow_ranger_2")

			-- Only applicable on creeps, except ancients
			if target:IsCreep() and not target:IsAncient() and target:HasModifier("modifier_imba_drow_ranger_marksmanship_723_proc_armor") and self:GetCaster():HasTalent("special_bonus_imba_drow_ranger_2") then
					target:Kill(self.ability, self:GetCaster())
			end
