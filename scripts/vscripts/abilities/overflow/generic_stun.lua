...
if generic_lua_silence == nil then
	generic_lua_silence = class({})
end

function generic_lua_silence:OnCreated( kv )	
	if IsServer() then
		if kv.stacking then
			self.stacking = kv.stacking
		else
...
function generic_lua_silence:OnRefresh( kv )	
	if IsServer() then
		if kv.stacking then
			self.stacking = kv.stacking
		else
...
function generic_lua_silence:IsDebuff()
	return true
end
 
function generic_lua_silence:IsStunDebuff()
	return true
end

 function generic_lua_silence:IsPurgable() 
	return true
end

function generic_lua_silence:IsPurgeException()
	return true
end
 
function generic_lua_silence:GetEffectName()
	return "particles/econ/items/drow/drow_arcana/drow_arcana_silenced.vpcf"
end
 
function generic_lua_silence:GetEffectAttachType()
	return PATTACH_OVERHEAD_FOLLOW
end
 

function generic_lua_silence:DeclareFunctions()
	local funcs = {
	MODIFIER_PROPERTY_OVERRIDE_ANIMATION 
	}
	return funcs
...
function generic_lua_silence:CheckState()
	local state = {
...
function generic_lua_silence:IsHidden()
	return false
end
