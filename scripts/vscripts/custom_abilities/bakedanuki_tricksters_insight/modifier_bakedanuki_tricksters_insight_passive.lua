...
function modifier_bakedanuki_tricksters_insight_passive:OnCreated( kv )
	-- references
	self.crit_chance = self:GetAbility():GetSpecialValueFor( "crit_chance" ) + self:GetCaster():GetTalentValue("special_bonus_unquie_bakedanuki_tricksters_insight_chance")
	self.crit_mult = self:GetAbility():GetSpecialValueFor( "crit_mult" ) + self:GetCaster():GetTalentValue("special_bonus_unquie_bakedanuki_tricksters_insight_mult")
end
