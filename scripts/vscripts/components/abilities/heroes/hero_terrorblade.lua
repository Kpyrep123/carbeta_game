...
LinkLuaModifier("modifier_imba_terrorblade_conjure_image_shard", "components/abilities/heroes/hero_terrorblade", LUA_MODIFIER_MOTION_NONE)
...
function imba_terrorblade_reflection:GetCooldown(level)
	return self.BaseClass.GetCooldown(self, level) - self:GetCaster():GetTalentValue("special_bonus_imba_terrorblade_reflection_cooldown")
end
...
function modifier_imba_terrorblade_reflection_slow:OnCreated(keys)
	...
	self.move_slow	= (self:GetAbility():GetSpecialValueFor("move_slow") + self:GetCaster():GetTalentValue("special_bonus_unquie_reflection_slow")) * (-1)
end
...
function imba_terrorblade_conjure_image:GetManaCost(Level)
	if self:GetCaster():HasTalent("special_bonus_unquie_conjure_nomana") then
		return 0
	else
...
function imba_terrorblade_conjure_image:OnSpellStart()
	...
	local illusions = CreateIllusions(self:GetCaster(), self:GetCaster(), {
		...
		outgoing_damage_structure	= nil,
		outgoing_damage_roshan		= nil,
		duration		= self:GetSpecialValueFor("illusion_duration") + self:GetCaster():GetTalentValue("special_bonus_unquie_counter_image_duration")
	}
	, 1, 108, false, true)
	...
	if illusions then
		for _, illusion in pairs(illusions) do
			...
			if self:GetCaster():HasShard() then
				self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_imba_terrorblade_conjure_image_shard", {duration = 5})
				illusion:AddNewModifier(self:GetCaster(), self, "modifier_imba_terrorblade_conjure_image_shard", {duration = 5})
			end
		end
	end
	if self:GetCaster():HasTalent("special_bonus_unquie_conjure_double") then 
...
modifier_imba_terrorblade_conjure_image_shard = class({})
function modifier_imba_terrorblade_conjure_image_shard:IsHidden() return false end
function modifier_imba_terrorblade_conjure_image_shard:IsPurgable() return false end
function modifier_imba_terrorblade_conjure_image_shard:GetEffectName() return "particles/econ/events/fall_2021/phase_boots_fall_2021.vpcf" end
function modifier_imba_terrorblade_conjure_image_shard:GetEffectAttachType(  ) return PATTACH_CUSTOMORIGIN end

function modifier_imba_terrorblade_conjure_image_shard:OnCreated()

end

function modifier_imba_terrorblade_conjure_image_shard:OnRefresh()

end

function modifier_imba_terrorblade_conjure_image_shard:DeclareFunctions()
return {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE,
		MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT}
end

function modifier_imba_terrorblade_conjure_image_shard:GetModifierAttackSpeedBonus_Constant(  )
	return 90
end

function modifier_imba_terrorblade_conjure_image_shard:GetModifierMoveSpeedBonus_Percentage(  )
	return 15
end
---------------------------------------------------------------
...
function modifier_imba_terrorblade_metamorphosis_transform:OnCreated()
	self.duration	= self:GetAbility():GetSpecialValueFor("duration") + self:GetCaster():GetTalentValue("special_bonus_unquie_meta_dur")
...
function imba_terrorblade_sunder:GetCooldown(level)
	if self.GetCursorTarget and self:GetCursorTarget() and self:GetCursorTarget():GetTeamNumber() ~= self:GetCaster():GetTeamNumber() and self:GetCursorTarget():IsMagicImmune() then
		return self.BaseClass.GetCooldown(self, level) - self:GetCaster():GetTalentValue("special_bonus_imba_terrorblade_sunder_cooldown") + self:GetSpecialValueFor("spell_immunity_cooldown_increase")
	else
		return self.BaseClass.GetCooldown(self, level) - self:GetCaster():GetTalentValue("special_bonus_imba_terrorblade_sunder_cooldown")
	end
end
