...
LinkLuaModifier( "modifier_forgotten_hero_herofall_trail", "heroes/forgotten_hero/modifiers/modifier_forgotten_hero_herofall_debuff", 	LUA_MODIFIER_MOTION_NONE )
...
function ability:GetCastRange(point, target)
	...
	return self.BaseClass.GetCastRange(self, point, target) + self:GetCaster():GetTalentValue("forgotten_hero_talent_herofall_range_bonus")
end
...
function ability:OnSpellStart( kv )
	...
	ParticleManager:CreateParticle("particles/units/heroes/hero_riki/riki_blink_strike_start_smoke_copy.vpcf", PATTACH_ABSORIGIN, caster)
