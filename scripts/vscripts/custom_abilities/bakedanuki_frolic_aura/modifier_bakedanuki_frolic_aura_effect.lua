...
function modifier_bakedanuki_frolic_aura_effect:OnCreated( kv )
	-- references
	self.evasion_self = self:GetAbility():GetSpecialValueFor( "evasion_self" ) + self:GetCaster():GetTalentValue("special_bonus_unquie_bakedanuki_futatsuiwas_curse_bonus_chance") -- special value
	self.evasion_ally = self:GetAbility():GetSpecialValueFor( "evasion_ally" ) + self:GetCaster():GetTalentValue("special_bonus_unquie_bakedanuki_futatsuiwas_curse_bonus_chance")-- special value
end
