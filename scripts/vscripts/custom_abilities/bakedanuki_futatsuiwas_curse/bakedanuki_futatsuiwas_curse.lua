...
function bakedanuki_futatsuiwas_curse:OnSpellStart()
	...
	-- load data
	local hexDuration = self:GetSpecialValueFor( "hex_duration" ) + self:GetCaster():GetTalentValue("special_bonus_unquie_bakedanuki_futatsuiwas_curse_dur")
	...
	target:AddNewModifier(
		...
		{ duration = hexDuration *(1 - target:GetStatusResistance()) } -- kv
	)
	...
	for _,enemy in pairs(enemies) do
		enemy:AddNewModifier(
			...
			self, -- ability source
			"modifier_bakedanuki_futatsuiwas_curse", -- modifier name
			{ duration = hexDuration *(1 - enemy:GetStatusResistance()) } -- kv
		)
