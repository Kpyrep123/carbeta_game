LinkLuaModifier("modifier_conviction", "scripts/vscripts/heroes/lathaal/conviction.lua", LUA_MODIFIER_MOTION_NONE)
...
function lathaal_conviction:OnSpellStart()
	...
	local modifier = target:AddNewModifier(caster, self, "modifier_conviction", {duration = self:GetSpecialValueFor("channel_time"), break_distance = self:GetSpecialValueFor("break_distance") + caster:GetCastRangeIncrease()})

	local particle = ParticleManager:CreateParticle("particles/units/heroes/hero_lathaal/conviction.vpcf", PATTACH_POINT_FOLLOW, caster)
