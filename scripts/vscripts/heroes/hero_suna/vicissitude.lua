...
function MindControlAdd( keys )
	...
	local duration = ability:GetLevelSpecialValueFor("duration", ability:GetLevel() - 1) + caster:GetTalentValue("spec_puppet_control_dur")
	caster:RemoveNoDraw()
	target:AddNoDraw()
...
function ApplyModifier( keys )
 ...
 local target = keys.target
 local ability = keys.ability
 local mod = "modifier_vicissitude_effect"
 local mod2 = "modifier_vicissitude"

 		local duration = ability:GetLevelSpecialValueFor("duration",  ( ability:GetLevel() - 1 )) + caster:GetTalentValue("spec_puppet_rope_dur") + caster:GetTalentValue("spec_puppet_control_dur")

	ability:ApplyDataDrivenModifier( caster, caster, mod, { Duration = duration } )
	ability:ApplyDataDrivenModifier( caster, target, mod2, { Duration = duration } )
end