...
function bakedanuki_tricksters_insight:OnSpellStart()
	...
	-- load data
	local bDuration = self:GetSpecialValueFor("crit_duration") + self:GetCaster():GetTalentValue("special_bonus_unquie_bakedanuki_tricksters_insight_dur")
	...
	target:AddNewModifier(
		...
		{ duration = bDuration*(1 - target:GetStatusResistance()) } -- kv
	)
