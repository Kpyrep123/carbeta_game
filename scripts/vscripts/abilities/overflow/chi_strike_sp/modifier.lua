...
function chi_strike_sp_mod:DoProjectile(hTarget)
	...
	EmitSoundOnLocationWithCaster(hCaster:GetAbsOrigin(), "Hero_Magnataur.ShockWave.Particle", hCaster)
	hCaster:EmitSound("Hero_Magnataur.ShockWave.Particle")
	EmitSoundOn("Hero_Magnataur.ShockWave.Particle", hCaster)
	local hAbility = self:GetAbility()
	local bonus_range = hCaster:GetCastRangeIncrease()
	local range = hAbility:GetCastRange(hCaster:GetAbsOrigin(),hAbility.original_target) + bonus_range + hCaster:GetTalentValue("special_bonus_unquie_chi_range")
	--local range = hAbility:GetSpecialValueFor("range")
	local speed = hAbility:GetSpecialValueFor("speed") + bonus_range + hCaster:GetTalentValue("special_bonus_unquie_chi_range")
	if not hCaster:HasShard() then
	local info =
	{
		Ability = hAbility,
