...
function modifier_fairy_queen_fairies:AllowIllusionDuplicate()
	return false
	-- body
end
...
function modifier_fairy_queen_fairies:CalculateCharge()
	if self:GetStackCount()>=self.max_fairies then
		...
		if self:GetRemainingTime() <= 0.05 then
			-- get charge time
			local charge_time = self:GetAbility():GetSpecialValueFor( "replenish_time" ) + self:GetCaster():GetTalentValue("spec_fairy_fairies_cooldown")
