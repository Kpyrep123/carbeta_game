...
function modifier_fairy_queen_enchantment_debuff:OnCreated( kv )
	-- references
	self.bonus = -self:GetAbility():GetSpecialValueFor( "speed_bonus" ) + self:GetCaster():GetTalentValue("spec_fairy_enchantment_slow")
	self.radius = self:GetAbility():GetSpecialValueFor( "radius" ) + self:GetCaster():GetTalentValue("spec_fairy_enchantment_radius")
