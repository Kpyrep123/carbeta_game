...
function modifier_huntress_crippling_arrow:OnAttackLanded(params)
    ...
    local rollChance = ability:GetSpecialValueFor("chance_root") + caster:GetTalentValue("huntress_crippling_arrow_chance_tallent")

    if params.target ~= nil and RollPercentage(rollChance) then
        local duration = ability:GetSpecialValueFor("duration_root") + caster:GetTalentValue("huntress_crippling_arrow_duration_tallent")
        ...
        if caster:HasModifier("modifier_huntress_hunting_spirit") and caster:HasTalent("huntress_hunting_spirit_heavy_arrow_tallent") then
            duration = duration / 100 * caster:GetTalentValue("huntress_hunting_spirit_heavy_arrow_tallent")
        end
