...
function spell_lab_ggs:Explosion(vPos)
  local hCaster = self:GetCaster()
	local iInt = hCaster:GetAttackDamage()
 local aoe = self:GetSpecialValueFor("radius") + hCaster:GetTalentValue("nova_modifier_bonus_radius")
   local damage = {
   ...
   EmitSoundOnLocationWithCaster(vPos, "Hero_Sniper.ShrapnelShatter", hCaster )
end
