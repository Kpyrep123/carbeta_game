...
for _,i in ipairs(ice) do 
    ...
    if i:GetUnitName() == "npc_lich_ice_unit" then 
        flag = true
    end
end
    ...
    and (thisEntity:GetAbsOrigin() - enemy_for_ability[1]:GetAbsOrigin()):Length2D()  <= thisEntity.ulti:GetCastRange(thisEntity:GetAbsOrigin(), thisEntity) 
        then 
            ...
            for _,i in ipairs(ice) do 
                 ...
                 if i:GetUnitName() == "npc_lich_ice_unit" and 
...
if #enemy_for_attack > 0 then 
    ...
    for i = 1,#enemy_for_attack do
         if enemy_for_attack[i] ~= nil and (enemy_for_attack[i]:IsCourier() or enemy_for_attack[i]:GetUnitName() == "npc_teleport") then 
    ...
    if #enemy_for_attack > 0 then 
         ...
         for i = 1,#enemy_for_attack do 
            if not enemy_for_attack[i]:IsInvulnerable() and not enemy_for_attack[i]:IsAttackImmune() then 
                n = i  
                break  
            end
         end
