...
function void_spirit_astral_step_lua:OnSpellStart()
	...
	local counter_buff = caster:FindModifierByName("modifier_warpath_datadriven_counter")
	local target = GetGroundPosition( origin + direction*dist + caster:GetCastRangeIncrease(), nil )

	-- teleport
	FindClearSpaceForUnit( caster, target, true )
	local subab = caster:FindAbilityByName("luna_void_blast")
	-- find units in line
	...
	for _,enemy in pairs(enemies) do
		...
		caster:PerformAttack( enemy, true, true, true, false, true, false, true )
			if not caster:HasModifier("modifier_warpath_datadriven_counter") then 

					local buffed = subab:ApplyDataDrivenModifier(caster, caster, "modifier_warpath_datadriven_counter", {}) 
					buffed:SetStackCount(1)
			end
		-- add modifier
		enemy:AddNewModifier(
			...
			{ duration = delay * (1 - enemy:GetStatusResistance()) } -- kv
		)
