...
function imba_pugna_nether_ward:OnSpellStart()
	...
	for i = 1, 1 + caster:GetTalentValue("special_bonus_imba_pugna_8") do
		...
		nether_ward:AddNewModifier(caster, ability, "modifier_kill", {duration = duration - duration * caster:GetTalentValue("special_bonus_imba_pugna_8","duration_reduce_pct") * 0.01})
...
function modifier_imba_nether_ward_degen:OnSpentMana(keys)
	if IsServer() then
		...
		if caster:HasTalent("special_bonus_imba_pugna_6") then
			ward:AddNewModifier(ward, nil, "modifier_pugna_decrepify", {duration = caster:GetTalentValue("special_bonus_imba_pugna_6")})
		end
