...
function JetStreamEffect( keys )
	...
	local abilityStackBuff = target:GetModifierStackCount("modifier_jet_stream_buff_increase", ability) + caster:GetTalentValue("zephyr_spec_3)")
	local abilityStackDebuff = target:GetModifierStackCount("modifier_jet_stream_debuff_decrease", ability) + caster:GetTalentValue("zephyr_spec_3)")
...
function JetStreamBuff( keys )
	...
	local ability = keys.ability
	local target = keys.target
	local stacks = ability:GetLevelSpecialValueFor("max_speed", ability:GetLevel() - 1) + caster:GetTalentValue("zephyr_spec_3)")
	...
	local abilityStack = target:GetModifierStackCount("modifier_jet_stream_buff_increase", ability) + caster:GetTalentValue("zephyr_spec_3)")
	...
	if not target:HasModifier("modifier_jet_shard_buff_increase") then
		ability:ApplyDataDrivenModifier(caster, target, "modifier_jet_shard_buff_increase", {} )
	end
	
	local abilityStack = target:GetModifierStackCount("modifier_jet_shard_buff_increase", ability) + caster:GetTalentValue("zephyr_spec_3)")
	if abilityStack < stacks and result_angle < 45 then
		target:SetModifierStackCount("modifier_jet_shard_buff_increase", ability, abilityStack + 1)
	end
end
end
...
function JetStreamDebuff( keys )
	...
	local abilityStack = target:GetModifierStackCount("modifier_jet_stream_debuff_decrease", ability) + caster:GetTalentValue("zephyr_spec_3)")
	if abilityStack < stacks and result_angle > 45 then
...
function JetStreamCheck( keys )
	...
	if not target:HasModifier("modifier_jet_stream_buff") then
		target:RemoveModifierByName("modifier_jet_shard_buff_increase")
	end
else
		if not target:HasModifier("modifier_jet_stream_buff") then
		target:RemoveModifierByName("modifier_jet_stream_buff_increase")
	end
end
end
