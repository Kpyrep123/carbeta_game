...
function warp_beast_warp:CanWarp(maxCastRange, castPosition, ability)
	local caster = self:GetCaster()
	local distancePerMana = self:GetSpecialValueFor("distance_per_mana") + caster:GetTalentValue("special_bonus_unique_warp_beast_warp_mana_pool")
...
function warp_beast_warp:Warp(maxCastRange, castPosition, ability, order)
	...
	if caster:HasTalent("special_bonus_unique_warp_beast_warp_silence") then
		local talent = caster:FindAbilityByName("special_bonus_unique_warp_beast_warp_silence")
		local units = FindUnitsInRadius(caster:GetTeamNumber(), warpPosition, nil, talent:GetSpecialValueFor("radius"), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)
		for k, unit in pairs(units) do
			unit:AddNewModifier(caster, self, "modifier_silence", {duration = talent:GetSpecialValueFor("value") - unit:GetStatusResistance()})
		end
	end
