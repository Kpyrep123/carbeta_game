...
function fairy_queen_quell:OnProjectileHitHandle( target, location, iProjectileHandle )
	...
	-- get data
	local damage = self:GetSpecialValueFor( "damage" ) + self:GetCaster():GetTalentValue("spec_fairy_damage")
	if fairies>=3 then
		damage = self:GetSpecialValueFor( "damage_strong" ) + self:GetCaster():GetTalentValue("spec_fairy_damage")
	end
