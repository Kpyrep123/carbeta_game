...
function hawkeye_detonator_dart:OnSpellStart()
	...
	local info = 
	  ...
	  Source = spawn_origin,
	  Ability = self,  
	  EffectName = "particles/items_fx/misery_projectile.vpcf",
...
function modifier_detonator_dart:OnCreated()
	if IsServer() then
		...
		local stack = self:GetAbility():GetSpecialValueFor("ticks") + self:GetCaster():GetTalentValue("dethmark_ticks")
...
function modifier_detonator_dart:OnIntervalThink()
	...
	local damage = self:GetAbility():GetSpecialValueFor("damage") + self:GetCaster():GetTalentValue("dethmark_damage")
	...
	if stack > 1 then
		...
		local radius = self:GetAbility():GetSpecialValueFor("radius") + self:GetCaster():GetTalentValue("dethmark_radius")
